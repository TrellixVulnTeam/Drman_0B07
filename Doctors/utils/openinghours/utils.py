...
from ...models import OpeningHours, ClosingRules , DoctorsPost
...
def get_closing_rule_for_now(location):
    ...
    if location:
        return ClosingRules.objects.filter(doctors=location,
                                           start__lte=now, end__gte=now)

    return DoctorsPost.objects.first().closingrules_set.filter(start__lte=now,
                                                           end__gte=now)
...
def is_open(location, now=None):
    ...
    if location:
        ohs = OpeningHours.objects.filter(doctors=location)
    else:
        ohs = DoctorsPost.objects.first().openinghours_set.all()
    for oh in ohs:
        is_open = False
